...
class Dashboard(ft.UserControl):
    ...
    def set_dashboard_value(self):
        ...
        for parameter in parameter_dict:
            if parameter == "engine_rpm":
                self._parameters_list.controls[1].content = self.create_text(
                    parameter, maintenance_dict
                )
                self._parameters_list.controls[1].update()
            if parameter == "lub_oil_pressure":
                self._parameters_list.controls[2].content = self.create_text(
                    parameter, maintenance_dict
                )
                self._parameters_list.controls[2].update()
            if parameter == "fuel_pressure":
                self._parameters_list.controls[3].content = self.create_text(
                    parameter, maintenance_dict
                )
                self._parameters_list.controls[3].update()
            if parameter == "coolant_pressure":
                self._parameters_list.controls[4].content = self.create_text(
                    parameter, maintenance_dict
                )
                self._parameters_list.controls[4].update()
            if parameter == "lub_oil_temp":
                self._parameters_list.controls[5].content = self.create_text(
                    parameter, maintenance_dict
                )
                self._parameters_list.controls[5].update()
            if parameter == "coolant_temp":
                self._parameters_list.controls[6].content = self.create_text(
                    parameter, maintenance_dict
                )
                self._parameters_list.controls[6].update()
            if parameter == "engine_condition":
                self._parameters_list.controls[0].content = self.create_text(
                    parameter, maintenance_dict
                )
    ...
    def create_text(self, parameter, maintenance_dict):
        ...
        if parameter == "engine_rpm":
            ...
            required_text.controls.append(
                ft.Icon(name=ft.icons.DIRECTIONS_CAR, size=40)
            )
            required_text.controls.append(
                ft.Text(
    ...
    def build(self):
        ...
        self._Dashboard = ft.Container(
            ...
            height=600,
            content=ft.Column(
                controls=[self._dashtitle, self._parameters_list],
            ),
